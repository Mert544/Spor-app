...
function descriptionPills(desc, max = 3) {
  if (!desc) return [];
  return desc.split('·').map((s) => s.trim()).filter(Boolean).slice(0, max);
}
          ...
          {activeTab === 'hazir' && (
            <div>
              ...
              {activeProgram && activeMeta && (
                <button
                  onClick={() => navigate(`/programlar/${activeMeta.id}`)}
                  className="w-full bg-bg-card rounded-2xl p-3 mb-4 flex items-center gap-3 border border-white/10 text-left transition-all active:scale-[0.99]"
                >
                  <div
                  ...
                  <span className="flex items-center gap-1.5 text-xs font-semibold text-white/70 px-2 py-1 rounded-full bg-white/[0.06] flex-shrink-0">
                    <span
                      className="w-2 h-2 rounded-full"
                      style={{ backgroundColor: LEVEL_DOT[activeLevel] || '#F5A623' }}
                    />
              ...
              <div className="space-y-3">
                {visiblePrograms.map((lib) => {
                  ...
                  const pills = descriptionPills(lib.description);
                  return (
                    <button
                      key={lib.id}
                      onClick={() => navigate(`/programlar/${lib.id}`)}
                      className="w-full bg-bg-card rounded-2xl border transition-all text-left active:scale-[0.99]"
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-1.5 mt-1.5">
                            {pills.map((pill, i) => (
                              <span
                                key={i}
                                className="text-xs px-2 py-0.5 rounded-full bg-white/[0.04] border border-white/[0.06] text-white/50"
                              >
                                {pill}
                              </span>
                            ))}
                    ...
                    {regularCustomList.map((cp) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 p-4">
                            <button
                              onClick={() => navigate(`/programlar/${cp.id}`)}
                              className="w-11 h-11 rounded-xl flex items-center justify-center text-2xl flex-shrink-0"
                            ...
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              ...
                              {confirmDelete === cp.id ? (
                                ...
                                <button
                                  ...
                                  className="w-8 h-8 flex items-center justify-center rounded-xl bg-white/5 text-white/30"
                                >
                                  <Trash2 size={14} />
                                </button>
                              )}
