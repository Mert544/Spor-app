  ...
  async function handleSubmit(e) {
    ...
    try {
      if (mode === 'signup') {
        ...
        if (data.session) {
          // Immediately active (email confirm disabled in Supabase settings)
          await pushAllData(data.session.user.id); // seed with local data
        ...
        if (err) throw err;
        setSession(data.session);
        // Data pull happens in App.jsx on session change
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col items-center justify-center px-6">
      ...
      <div className="mb-8 text-center">
        ...
        <p className="text-sm text-white/40 mt-1">Antrenmanını bulutla yedekle</p>
      </div>
      ...
      <div className="w-full max-w-sm bg-bg-card rounded-3xl p-6 border border-white/6">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {/* Error / success */}
          {error && (
            <p className="text-xs text-[#E94560] bg-[#E94560]/10 rounded-xl px-3 py-2">{error}</p>
          )}
          {success && (
            <p className="text-xs text-[#14B8A6] bg-[#14B8A6]/10 rounded-xl px-3 py-2">{success}</p>
          )}
          ...
          <button
            ...
            style={{
              backgroundColor: (loading || !email.trim() || password.length < 6)
                ? '#1e293b' : '#14B8A6',
          ...
          >
            {loading
              ...
              : mode === 'signup' ? 'Kayıt Ol & Senkronize Et' : 'Giriş Yap'
            }
          </button>
        </form>
