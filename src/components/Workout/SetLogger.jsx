...
export default function SetLogger({ date, exerciseId, setIndex, accentColor }) {
  ...
  return (
    <div className={`transition-colors ${done ? 'opacity-60' : ''}`}>
      <div className="flex items-center gap-2 py-1.5 px-1 rounded-lg">
      ...
      {/* 1RM göstergesi */}
      {estimated1RM && weight && reps && !done && (
        <div className="flex justify-end pr-1 pb-1">
          <span className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: `${accentColor}18`, color: accentColor }}>
            ~1RM: {estimated1RM} kg
          </span>
        </div>
      )}
